 ...
 *     Jesper S Moller <jesper@selskabet.org> - Contributions for bug 378674 - "The
 * method can be declared as static" is wrong Andy Clement (GoPivotal, Inc)
 * aclement@gopivotal.com - Contributions for Bug 383624 - [1.8][compiler]
 * Revive code generation support for type annotations (from Olivier's work) Bug
 * 409245 - [1.8][compiler] Type annotations dropped when call is routed through
 * a synthetic bridge method Till Brychcy - Contributions for bug 413460 -
 * NonNullByDefault is not inherited to Constructors when accessed via Class
 * File Lars Vogel <Lars.Vogel@vogella.com> - Contributions for Bug 473178
 ******************************************************************************
 */
...
public class AllocationExpression extends Expression implements IPolyExpression, Invocation {
    ...
    private SimpleLookupTable/*<PMB,IC18>*/ inferenceContexts = null;
...
// For allocation expressions, boxing compatibility is same as vanilla compatibility, since java.lang's wrapper types are not generic.
    ...
    public void checkTypeArgumentRedundancy(ParameterizedTypeBinding allocationType, final BlockScope scope) {
        if ((scope.problemReporter().computeSeverity(IProblem.RedundantSpecificationOfTypeArguments) == ProblemSeverities.Ignore) || scope.compilerOptions().sourceLevel < ClassFileConstants.JDK1_7) {
            return;
        }
    ...
    /**
     * @see
     * org.eclipse.jdt.internal.compiler.ast.Expression#setExpectedType(org.eclipse.jdt.internal.compiler.lookup.TypeBinding)
     */
