 ...
 *     Stephen Herrmann <stephan@cs.tu-berlin.de> - Contributions for bug 133125 -
 * [compiler][null] need to report the null status of expressions and analyze
 * them simultaneously bug 292478 - Report potentially null across variable
 * assignment bug 324178 - [null] ConditionalExpression.nullStatus(..) doesn't
 * take into account the analysis of condition itself bug 354554 - [null]
 * conditional with redundant condition yields weak error message bug 349326 -
 * [1.7] new warning for missing try-with-resources bug 345305 -
 ...
 * diagnostic Bug 392099 - [1.8][compiler][null] Apply null annotation on types
 * for null analysis Bug 415043 - [1.8][null] Follow-up re null type annotations
 * after bug 392099 Bug 417295 - [1.8[[null] Massage type annotated null
 * analysis to gel well with deep encoded type bindings. Bug 400874 -
 * [1.8][compiler] Inference infrastructure should evolve to meet JLS8 18.x
 * (Part G of JSR335 spec) Bug 426078 - [1.8] VerifyError when conditional
 * expression passed as an argument Bug 427438 - [1.8][compiler] NPE at
 * org.eclipse.jdt.internal.compiler.ast.ConditionalExpression.generateCode(ConditionalExpression.java:280)
 * Bug 418537 - [1.8][null] Fix null type annotation analysis for poly
 * conditional expressions Bug 428352 - [1.8][compiler] Resolution errors don't
 * always surface Bug 407414 - [compiler][null] Incorrect warning on a primitive
 * type being null
 * *****************************************************************************
 */
            ...
            if (TypeBinding.notEquals(expressionType, expectedTypeLocal)) {
                ...
                if (!expressionType.isCompatibleWith(expectedTypeLocal)) {
                    if (scope.isBoxingCompatibleWith(expressionType, expectedTypeLocal)) {
                        ...
                        if (mb2 != null) {
                            scope.problemReporter().abortDueToInternalError("Type " + this.condition.resolvedType.debugName()
                                    + //$NON-NLS-1$
