...
public class MaxjDocTask extends Task{
	...
	private static final String FAIL_MSG = "Javadoc failed; see the tools error output for details.";  //$NON-NLS-1$
	...
	public void execute() throws BuildException{
		...
		try {
			...
			for(int i = 0; i <argumentsList.size(); i++){
				arguments[i] = (String)argumentsList.get(i);
			...
			FileWriter outFile = new FileWriter(fileList);
			...
			ls_in.close();
			ls_in = new BufferedReader( new InputStreamReader(ls_proc.getErrorStream()));
			try {
				while ((ls_str = ls_in.readLine()) != null) {
					System.out.println(ls_str);
			    }
	...
	private void processArguments(ArrayList arguments) {
		...
		if(this.doclet != null){
			if(!this.doclet.getName().equals("")){ //$NON-NLS-1$
				...
				ArrayList params = this.doclet.getParams();
				for(int i = 0; i < params.size(); i++){
					arguments.add(((DocletParam)params.get(i)).getName());
					arguments.add(((DocletParam)params.get(i)).getValue());
				}
			}
		}
...
//			arguments.add(OPTION_PREFIX + ADDITIONALPARAM_OPTION);
		...
		if(this.links.size() > 0){
			for(int i = 0; i < this.links.size();i++){
				...
				arguments.add(((LinkArgument)this.links.get(i)).getHref());
			}
		}
	...
	private void packagesToSrcFiles() {
		...
		for(int i = 0; i < srcPaths.length; i++){
			...
			for(int j = 0; j < this.packages.size(); j++){
				File file = new File(srcPaths[i] + (String)this.packages.get(j));
				if(file.exists()){
	...
	private void parsePath() {
		...
		for(int i = 0; i < pathElements.length; i++){
			...
			if(pathElement.isDirectory()){
				...
				if(!this.srcFiles.contains(pathElement))
					this.srcFiles.add(pathElement);
			}
		}
	}
