...
public class MaxjTask extends Task{
		...
		private static final String FAIL_MSG = "Compile failed; see the compiler error output for details.";  //$NON-NLS-1$

		private final static String EMPTY_STRING = ""; //$NON-NLS-1$
		private final static String JAVA_VERSION = "1.6"; //$NON-NLS-1$
		...
		private final ArrayList compilersArg;
		private final ArrayList filesAndFolders;

		protected Map customDefaultOptions;
			...
			try {
				...
				if(this.listFiles){
					...
					for(int i = 0; i < this.filesAndFolders.size();i++){
						logFolder(new File((String)this.filesAndFolders.get(i)));											
					}
					log("",Project.MSG_INFO);//$NON-NLS-1$
				}
				...
				for(int i = 0; i < this.arguments.size(); i++){
					argStringArray[i] = (String)this.arguments.get(i);
		...
		protected void createArgumnetsLine() {
			...
			for(int i = 0; i < this.compilersArg.size(); i++){
				this.arguments.add(((CompileargTask)this.compilersArg.get(i)).getValue());
			}
			if (this.target != null) {
